...
public partial class Remisiones : System.Web.UI.Page
{
    ...
    protected void lnkGeneraRemision_Click(object sender, EventArgs e)
    {
        ...
        try {
            int[] sesiones = obtieneSesiones();
            object[] completada = recepciones.obtieneTodoAplicado(sesiones);
            ...
            if (Convert.ToBoolean(completada[0]))
            {
                if (Convert.ToInt32(completada[1]) != completados)
                    lblError.Text = "No es posible generar la remisión ya que existen conceptos de mano de obra o de refacciones aun sin aplicar";
                else {
                    if (completados == 0)
                        lblError.Text = "No se puede generar la remisión ya que no existen conceptos de mano de obra o de refacciones registrados";
                    else
                    {
                        ...
                        {                           
                            ...
                            lblError.Text = "Se ha generado la remisión " + argumentos[0] + " con número: " + argumentos[1];
                            RadGrid1.Rebind();
                            RadGrid2.Rebind();
                            RadGrid3.Rebind();
                            txtObs.Text = "";
        ...
        catch (Exception ex) { lblError.Text = "Error al generar la remision. Detalle: " + ex.Message; }
    }
    ...
    protected void lnkImprime_Click(object sender, EventArgs e)
    {
        ...
        if (remision != 0)
        {
            ...
            lblError.Text = "Debe seleccionar una remisión para imprimirla";
    }
}