...
public partial class Grupo_Operacion_Cat : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        catch (Exception ex)
        {
            ...
            Session["paginaOrigen"] = "Grupo_Operacion_Cat.aspx";
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (modo != "Edit")
            {
                ...
                try
                {
                    ...
                    {
                        if (Convert.ToInt32(GridGrupoOp.DataKeys[e.Row.RowIndex].Value.ToString()) < 4)
                        {
    ...
    protected void GridGrupoOp_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        try
        {
            ...
            if (Convert.ToBoolean(valores[0]))
            {
                ...
                {
                    e.Cancel = true;
                    lblError.Text = "El grupo no se puede eliminar ya que se encuentra ocupado en otro proceso";
                }
            }
            else
                lblError.Text = "El grupo no se logro eliminar correctamente, verifique su conexión e intentelo nuevamente mas tarde";
        }
        catch (Exception x )
        {
            e.Cancel = true;
